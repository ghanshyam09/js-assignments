...
function getRectangleString(width, height) {
  ...
  for (let i = 0; i < height; i++) {
    if (i === 0) {
      res += map.tleft + map.hr.repeat(width - 2) + map.tright + "\n";
    } else if (i == height - 1) {
      res += map.bleft + map.hr.repeat(width - 2) + map.bright + "\n";
    } else {
      res += map.vr + " ".repeat(width - 2) + map.vr + "\n";
    }
  }
  return res;
}
...
function encodeToRot13(str) {
  ...
  for (let i = 0; i < str.length; i++) {
    ...
    if (96 < index && index < 123) {
      index = str.charCodeAt(i) + 13;
      if (index > 122) {
        res += String.fromCharCode(96 + (index - 122));
      } else {
        res += String.fromCharCode(index);
      }
    } else if (64 < index && index < 91) {
      index = str.charCodeAt(i) + 13;
      if (index > 90) {
        res += String.fromCharCode(64 + (index - 90));
      } else {
        res += String.fromCharCode(index);
      }
    } else {
